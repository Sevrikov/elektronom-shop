  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        if (res?.error) {
          setError(uk ? 'Невірний email або пароль' : 'Неверный email или пароль')
        } else {
        ...
        setError(uk ? 'Сталася помилка входу' : 'Произошла ошибка входа')
      }
    })
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="text-center mb-2">
        <h2 className="text-xl font-bold text-text-primary">
          {uk ? 'Вхід до акаунту' : 'Вход в аккаунт'}
        </h2>
        <p className="text-xs text-text-muted mt-1">
          {uk ? 'Введіть ваші дані для входу' : 'Введите ваши данные для входа'}
        </p>
      </div>
      ...
      >
        {isPending ? <Loader2 className="size-4 animate-spin" /> : (uk ? 'Увійти' : 'Войти')}
      </button>
      ...
      <p className="text-center text-xs text-text-muted mt-2">
        {uk ? 'Немає акаунту?' : 'Нет аккаунта?'}{' '}
        <Link href={`/${locale}/register` as never} className="text-accent font-semibold hover:underline">
          {uk ? 'Зареєструватися' : 'Зарегистрироваться'}
        </Link>
      </p>
    </form>
  )
